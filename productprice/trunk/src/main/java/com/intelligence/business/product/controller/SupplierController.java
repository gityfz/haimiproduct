...
public class SupplierController extends BaseController {
	...
	public void selectSupplier(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			// 获取供应商id
			Integer supplierId = Integer.valueOf(request.getParameter("supplierId"));
			// 记录日志
			logger.info(LogUtils.commonFormat("查询一条供应商接口入参", supplierId,
					"/supplier/select", "get"));
			// 校验入参
			String paramsMsg = StringCon.EMPTY_STR.getValue();
			...
			if (!StringCon.EMPTY_STR.getValue().equals(paramsMsg)) {
				...
				// 查询供应商信息
				Supplier supplierMap = iSupplierService.selectSupplier(supplierId);
				// 返回结果
				this.sendResult(response, supplierMap);
			}
